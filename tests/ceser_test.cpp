...
TEST_F(ReadIntTest, ReadU8) {
    uint8_t u8 = read_u8(buf, 0);
    EXPECT_EQ(u8, 0xaa);
}
...
TEST_F(WriteIntTest, WriteU8) {
    int ret = write_u8(0xaa, buf);
    EXPECT_EQ(ret, 1);
...
TEST_F(WriteIntTest, WriteI8) {
    int ret = write_i8(0xaa, buf);
    EXPECT_EQ(ret, 1);
    EXPECT_EQ(buf[0], 0xaa);
}
